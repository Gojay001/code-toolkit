...
from model.densenet_ms import densenet161
...
class JL_DCF(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = [self.score_DCF(x_fa[i]) for i in (0, -1)]
        return mask, x_fa
...
if __name__ == '__main__':
    ...
    depth = torch.rand(1, 3, 320, 320)
    model = build_model('vgg16', 'vgg')
    input = torch.cat((images, depth), dim=0)
    preds, pred_coarse = model(input)
    print(preds.size(), pred_coarse.size())
