...
from data_loader import get_folders, MyDataSet, TestDataSet, get_img_dict
...
def main():
    ...
    test_loader = DataLoader(dataset=test_datasets, batch_size=config.batch_size, shuffle=False)
    ...
    with torch.no_grad():
        for i, images in enumerate(test_loader, 0):
            ...
            for j in range(predicted.size(0)):
                results[i*config.batch_size+j]['disease_class'] = predicted[j].item()
