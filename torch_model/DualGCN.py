...
class DualGCN(nn.Module):
    ...
    def forward(self, x_feat, xe_feat):
        # # # # Local # # # #
        # local = self.local(x_feat)
        local = self.gcn_local_attention(x_feat)
        # local = F.interpolate(local, size=x.size()[2:], mode='bilinear', align_corners=True)
        spatial_local_feat = x_feat * local + x_feat

        # # # # Projection Space # # # #
        x_sqz, b = x_feat, xe_feat
...
if __name__ == '__main__':
    x_feat = torch.rand((1, 32, 320, 320), requires_grad=True)
    xe_feat = torch.rand((1, 32, 320, 320), requires_grad=True)
    model = DualGCN(32)
    out = model(x_feat, xe_feat)
    print(out.shape)
