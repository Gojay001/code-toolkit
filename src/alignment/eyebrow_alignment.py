...
def get_crop_rect_dlib(face_landmarks, expansion_ratio=1.0):
    ...
    lm_nostrils = lm[31: 36]  # left-right
    ...
    brow_left = np.mean(lm_eyebrow_left, axis=0)
    brow_right = np.mean(lm_eyebrow_right, axis=0)
    brow_avg = (brow_left + brow_right) * 0.5

    brow_left_to_brow_right = lm[26] - lm[17]

    # Choose oriented crop rectangle.
    x = eye_to_eye - np.flipud(eye_to_mouth) * [-1, 1]
    x /= np.hypot(*x)

    x *= 0.6 * np.hypot(*brow_left_to_brow_right)
    x *= expansion_ratio

    y = np.flipud(x) * [-1, 1] * 0.5

    c = (eye_avg + brow_avg) * 0.5
...
def get_crop_rect_bvt(face_landmarks, expansion_ratio=1.0):
    # bvt landmarks
    lm = np.array(face_landmarks)
    ...
    brow_left = np.mean(lm_eyebrow_left_up, axis=0)
    brow_right = np.mean(lm_eyebrow_right_up, axis=0)
    brow_avg = (brow_left + brow_right) * 0.5

    brow_left_to_brow_right = lm[42] - lm[33]

    # Choose oriented crop rectangle.
    x = eye_to_eye - np.flipud(eye_to_mouth) * [-1, 1]
    x /= np.hypot(*x)

    brow_left_length = np.hypot(*(lm[37] - lm[33]))
    brow_right_length = np.hypot(*(lm[42] - lm[38]))

    x *= 1.5 * max(brow_left_length, brow_right_length)
...
def image_align_run(img, face_landmarks, gt_img = None, mask_img = None,  detector='dlib', output_size=1024, transform_size=4096, enable_padding=False, expansion_ratio=1.0):
    # Align function from FFHQ dataset pre-processing step
    quad, qsize = get_crop_rect_dlib(face_landmarks, expansion_ratio) if detector == 'dlib' else get_crop_rect_bvt(face_landmarks, expansion_ratio)
